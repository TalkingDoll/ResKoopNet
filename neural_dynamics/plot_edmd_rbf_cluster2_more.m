...
%     resDMD_feature_reshaped = reshape(resDMD_feature, 300, 1301, Ntrial)
    resDMD_feature_reshaped = normalize(abs(efuns_mouse_trials), 2, 'range');

    all_resDMD_efuns = nan(1301,300,6,10);
    ...
    figure('Position', [873, 899,2215,379]);
    subplot(8,1,1)
    imagesc(1:6); 
    ...
    subplot(8,1,[2:8])
    imagesc(reshape(ave_resDMD_efuns,1301 ,6*300 ));
    xline(300:300:1800, 'k', 'LineWidth', 2)
    ...
    ylabel('Dim. 2');
    zlabel('Dim. 3');
    title(strcat('Mouse-', int2str(mouse_idx), ', EDMD+RBF (MDS)'));
    % title(strcat('resDMD'));
    ...
    ylabel('Dim. 2');
    zlabel('Dim. 3');
    title(strcat('Mouse-', int2str(mouse_idx), ', EDMD+RBF (UMAP)'));
    % title(strcat('resDMD'));
    ...
    ylabel('Dim. 2');
    zlabel('Dim. 3');
    title(strcat('Mouse-', int2str(mouse_idx), ', EDMD+RBF (t-SNE)'));
    % title(strcat('resDMD'));
